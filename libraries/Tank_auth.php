...
class Tank_auth
{
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						if ($user->activated == 0) {							// fail - not activated
							...
							$this->ci->users->update_profile($user->id,array('sid'=>$uniqueID),false);
							
							if($user->update_profile_notification==1){
								set_cookie(array('name'   => 'editprofilenow',
									'value'  => 1,
									'expire' => '1296000')
								);
							}
	...
	{
		$r = $this->ci->session->userdata('status') === ($activated ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED);
		
		if($r && $activated){
			$u = $this->ci->users->get_user_by_id($this->ci->session->userdata('user_id'),true);
			if(!empty($u->sid) && $u->sid==$this->ci->session->userdata('uniqueID'))return TRUE;
			else 
			{
				$this->logout();
				return FALSE;
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $email_activation,$first_name,$last_name,$country)
	{
